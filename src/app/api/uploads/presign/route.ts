    ...
    try {
      ...
      if (event.status !== 'ACTIVE') {
        return NextResponse.json(
          { success: false, error: 'El evento no está activo' },
          { status: 400 }
        );
      }
      ...
      if (new Date() > event.uploadWindowEnd) {
        return NextResponse.json(
          { success: false, error: 'La ventana de subida ha expirado' },
          { status: 400 }
        );
      }
      ...
      if (uploadCount >= MAX_PHOTOS_PER_GUEST) {
        return NextResponse.json(
          { success: false, error: `Límite de ${MAX_PHOTOS_PER_GUEST} fotos alcanzado` },
          { status: 400 }
        );
      }
      ...
      return NextResponse.json(
        { success: false, error: 'Error al preparar la subida' },
        { status: 500 }
      );
    }
  });
