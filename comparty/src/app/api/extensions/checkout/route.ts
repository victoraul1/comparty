  ...
  return withAuth(request, async (req, user) => {
    try {
      ...
      if (event.planTier === 'FREE') {
        return NextResponse.json(
          { success: false, error: 'Free plan does not allow extensions' },
          { status: 400 }
        );
      }
      ...
      if (activeExtension) {
        return NextResponse.json(
          { success: false, error: 'You already have an active extension' },
          { status: 400 }
        );
      }
      ...
      return NextResponse.json(
        { success: false, error: 'Error processing extension' },
        { status: 500 }
      );
    }
  });
...
export async function GET(request: NextRequest) {
  return withAuth(request, async (req, user) => {
    try {
      ...
      if (!eventId) {
        return NextResponse.json(
          { success: false, error: 'Event ID not provided' },
          { status: 400 }
        );
      }
      ...
      return NextResponse.json(
        { success: false, error: 'Error verifying extension' },
        { status: 500 }
      );
    }
  });
