...
export class PhotoProcessorService {
  static async processUpload(
    ...
    try {
      // Get upload and event information
      ...
      // 4. AI analysis (if enabled and plan allows)
      let aiAnalysis = null;
      ...
      // 7. Update upload with thumbnail and duplicate information
      ...
      // 8. Update automatic selection (Top 5)
  ...
  static async updateAutoSelection(eventId: string, uploaderId: string): Promise<void> {
    ...
    // Remove previous selections not pinned by host
  ...
  static async generateEventInsights(eventId: string): Promise<void> {
    ...
    if (aiAnalyses.length > 0) {
      ...
      // Here you could save the summary to the database or send it by email
    }
  }
}